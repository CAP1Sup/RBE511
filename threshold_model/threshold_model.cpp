...
struct GetRobotData : public CBuzzLoopFunctions::COperation {
  ...
  virtual void operator()(const std::string& str_robot_id, buzzvm_t t_vm) {
    ...
    if (!buzzobj_isint(tTask)) {
      LOGERR << str_robot_id << ": variable 'task' has wrong type "
             << buzztype_desc[tTask->o.type] << std::endl;
      return;
    }
    /* Get the value */
    ...
    if (nTask < 0 || nTask >= m_vecTaskCounts.size()) {
      LOGERR << str_robot_id << ": variable 'task' has wrong value " << nTask
             << std::endl;
      return;
    }
    ...
    if (!buzzobj_istable(tThreshold)) {
      LOGERR << str_robot_id << ": variable 'threshold' has wrong type "
             << buzztype_desc[tThreshold->o.type] << std::endl;
      return;
    }
    /* Get the values */
    ...
    for (int i = 0; i < m_vecTaskCounts.size(); ++i) {
      ...
      if (!buzzobj_isfloat(tThresholdValue)) {
        LOGERR << str_robot_id << ": element 'threshold[" << i
               << "]' has wrong type " << buzztype_desc[tThresholdValue->o.type]
               << std::endl;
      } else {
        /* Get the value */
...
struct PutStimuli : public CBuzzLoopFunctions::COperation {
  /** Constructor */
  PutStimuli(const std::vector<Real>& vec_stimuli)
      : m_vecStimuli(vec_stimuli) {}
...
void CThresholdModel::Destroy() { m_cOutFile.close(); }
...
void CThresholdModel::PostStep() {
  ...
  for (int i = 0; i < m_vecStimuli.size(); ++i) {
    m_vecStimuli[i] +=
        m_fDelta - m_fAlpha / GetNumRobots() * cGetRobotData.m_vecTaskCounts[i];
  ...
  if (!cGetRobotData.m_vecRobotsThresholds.empty()) {
    for (int i = 0; i < GetNumRobots(); ++i) {
      m_cOutFile << GetSpace().GetSimulationClock() << "\t" << i << "\t"
...
int CThresholdModel::GetNumRobots() const { return m_mapBuzzVMs.size(); }
